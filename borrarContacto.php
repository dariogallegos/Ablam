        ...
        <div class="col-md-12">
                          ...
                          echo'<div class="card border-secondary">';
                            echo'<li class="list-group-item">';
                              ...
                              echo'<i class="fa fa-fw fa-address-book-o fa-lg"></i> Direccion </li>';
                                ...
                                echo '<button type="submit" name ="borrar"class="btn my-2 text-center btn-block btn-secondary"> Borrar  </button>';
                                echo '</form>';
                          echo'</div>';
                        echo'</div>';
                      echo'</div>';
            ...
            <?php
              if(isset($_POST['borrar'])){
                $idContacto = $_SESSION['idContacto'];
                $idUsuario = $_SESSION['id'];
                $sql = "DELETE from contactos where idContacto = $idContacto ";
                $sqlCU = "DELETE from `usuarios-contactos` where idContacto = $idContacto and idUsuario = $idUsuario";
                $mysqli = conexion::getConection();
                $query = mysqli_query($mysqli,$sql);
                $queryCU = mysqli_query($mysqli,$sqlCU);
                if( $query and $queryCU){
                    echo '<div class="row">
                              <div class="col-md-4"></div>
                              <div class="col-md-4">
                                <div class="col-md-12">
                                  <div class="card w-100 h-25 bg-secondary border-secondary">
                                    <h3 class="mb-4 text-dark text-center">Se ha borrado el contacto</h3>
                                  </div>
                                </div>
                              </div>
                              <div class="col-md-4"></div>
